...
install:
    cargo install --path bin/reth --bin xlayer-reth --force --locked --profile release

install-maxperf:
    RUSTFLAGS="-C target-cpu=native" cargo install --path bin/reth --bin xlayer-reth --force --locked --profile maxperf --features jemalloc,asm-keccak
