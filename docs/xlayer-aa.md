...
**What I did.** Hit `panicked ... wait_cloned must not be called from a rayon worker thread` while running 0x7B end-to-end against a debug `xlayer-reth-node --chain xlayer-dev --dev` node built against the sibling `okx-reth` workspace at `feat/xlayer-aa-v1-1.11.3`. Traced the stack — `AccountProofWorker::run → database_provider_ro → OverlayStateProviderFactory::get_overlay → LazyOverlay::compute → wait_cloned` — and went looking for a structural fix. Considered: (1) moving proof workers off the rayon pool, (2) pre-warming `LazyOverlay::inner` on the caller thread via `database_provider_ro`'s side-effect, (3) adding an explicit `LazyOverlay::warm()` + `DatabaseProviderROFactory::warm_overlay_source()` default method so the proof-worker pool constructor calls it before spawning. Settled on (3) and started drafting the PR.

**Why it's wrong.** Before writing any code I should have checked `git log` in the paradigm upstream checkout for the assert's own history. Paradigm introduced the `debug_assert!` in PR #22492 ("fix: overlay preparation on tokio", `c685842ba`, 2026-02-23) as a guardrail alongside the real fix (switching a background `overlay.get()` call from `rayon::spawn` to `tokio::task::spawn_blocking` in `crates/engine/tree/src/tree/mod.rs`). Within 24 hours they realised the guard was over-broad — it fired on legitimate cross-pool callers like the proof workers — and removed it outright in PR #22505 ("fix: remove `debug_asssert!`", `98c45a424`, 2026-02-24). The assert exists in v1.11.3 only because the tag was cut in that 24-hour window; main / v2.0.0+ no longer carry it. So okx-reth `dev` (v1.10.1 base) is unaffected on two counts (no assert, and proof workers still on `spawn_blocking` there); okx-reth `feat/xlayer-aa-v1-1.11.3` inherits the window. No structural work is needed — paradigm's own judgment matches mine about the assert being too strict, and their fix is a three-line delete.

**Fix.** On the 1.11.3 line, cherry-pick paradigm `98c45a424` (PR #22505) into the affected okx-reth branch and note "backport paradigm #22505" in the commit message. Don't invent `warm_overlay_source()` — it solves nothing that paradigm hasn't already solved by deletion, and it would conflict with future upstream syncs. For the immediate unblock on a debug build, running the node with `CARGO_PROFILE_DEV_DEBUG_ASSERTIONS=false` or switching to a release build also sidesteps it without code changes.

**Rule going forward.** Before architecting any fix for a panic that originates in an upstream-tracked crate, `git log -S <unique-string>` in the upstream checkout for the exact assertion message or added code. Paradigm's own PR history often already contains both the bug report and the resolution — you either get a ready-made backport (one `git cherry-pick`) or you get a design confirmation (paradigm tried X, it didn't work, they did Y). Either way it's cheaper than re-deriving the fix from first principles.
