...
[Rule] Extend Node-Wide Behavior via Builder-Field Threading, Not Component Wrapping: To add per-transaction interception or node-wide behavior, thread a config/runtime context as a plain `FlashblocksBuilderCtx` field (the `bridge_intercept_config` / `blacklist_ctx` pattern) consumed at a post-execution hook in `crates/builder/src/flashblocks/context.rs`. Do NOT wrap upstream node components (custom `PoolBuilder` via `.pool()`, `ExecutorBuilder` cascade via `.executor()`, or a wrapper `ConfigureEvm` type) — Reason: upstream `OpAddOns`/`NodeAddOns`, payload service builders, and the engine validator are pinned to concrete types (`OpTransactionValidator`/`OpEvmConfig`); wrapping them fails to compile (E0277/E0308), discoverable only after a full ~1197-crate build. See pitfalls/upstream-component-type-pinning.
