...
// (the topology is identical across them) and run as subtests, because spinning a
// devnet up costs far more than any individual RPC assertion.
func TestRPC(gt *testing.T) {
	t := devtest.SerialT(gt)
	sys := presets.NewXLayer(t, xlayerBinaryOpts(t)...)
	...
	// NewTransactionTypes: the typed-transaction RPC surface. It asserts the
	// fee-history endpoint (EIP-1559 support) responds; the remaining EIP-cased
	// variants from the Rust suite build on the same submission path and are
	// tracked as follow-up cases once devnet execution is wired.
	gt.Run("NewTransactionTypes", func(gt *testing.T) {
