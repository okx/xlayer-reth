...
xlayer-reth init --datadir testnet-data --chain genesis-testnet-reth.json
echo "ℹ️ Importing testnet. Please wait patiently ..."
time xlayer-reth import --datadir testnet-data --chain xlayer-testnet --exported-data $TESTNET_EXPORT_FILE > testnet-import.log 2>&1
if [ $? -ne 0 ]; then
...
time xlayer-reth export --datadir testnet-data --chain xlayer-testnet --exported-data $TESTNET_REEXPORTED_FILE --start-block 12241700 > testnet-export.log 2>&1
if [ $? -ne 0 ]; then
