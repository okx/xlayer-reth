module github.com/okx/xlayer-reth/tests

go 1.26.0

toolchain go1.26.4

// The Go E2E harness consumes the vendored Optimism devstack and op-service
// helpers directly from the repository checkout so the tests build against the
// exact submodule commit pinned by deps/optimism, never a published version.
replace github.com/ethereum-optimism/optimism => ../deps/optimism

// Go replace directives are not transitive, so the harness must repeat the
// go-ethereum fork replacement declared by deps/optimism; without it op-service
// resolves the upstream go-ethereum and fails to compile against OP Stack types.
...
require (
	...
	github.com/ethereum-optimism/superchain-registry/validation v0.0.0-20260611202829-ac4e48516794 // indirect
	...
	gitlab.okg.com/okcoin-commons/ok-kms-go v1.0.9 // indirect
