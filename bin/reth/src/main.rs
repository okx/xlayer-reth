...
use xlayer_reth_export::export::ExportCommand;
use xlayer_reth_import::import::ImportCommand;
...
#[derive(Debug, Parser)]
#[command(author, version, about = "XLayer Reth", long_about = None)]
struct XLayerCli {
    #[command(subcommand)]
    command: Commands,
}
...
enum Commands {
    ...
    /// Initialize the XLayer node
    #[command(name = "init")]
    Init(InitCommand<XLayerChainSpecParser>),

    /// Import blocks from a file in RLP format
    #[command(name = "import")]
    Import(ImportCommand<XLayerChainSpecParser>),

    /// Export blocks to a file in RLP format
    #[command(name = "export")]
...
struct NodeCommand {
    #[command(flatten)]
    cli: Cli<XLayerChainSpecParser, Args>,
}
...
async fn main() -> ExitCode {
    ...
    match cli.command {
        Commands::Node(node_cmd) => {
            let result = node_cmd.cli.run(|builder, args| async move {
        ...
        Commands::Init(init_cmd) => {
            let result = init_cmd.execute::<OpNode>().await;
            info!(message = "initializing XLayer node");
            match result {
                Ok(_) => ExitCode::SUCCESS,
                Err(e) => {
                    error!(target: "xlayer::init", "Error: {:#?}", e);
                    ExitCode::FAILURE
                }
            }
        }
        Commands::Import(import_cmd) => {
            ...
            match import_cmd.execute::<OpNode, _>(components).await {
                Ok(_) => ExitCode::SUCCESS,
                Err(e) => {
