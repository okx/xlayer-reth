#!/usr/bin/env bash
# Run N rounds × {nojit, jit_cold, aot} of the SA-Bench paid-fb-contract path.
# Skips AOT prewarmup if cache is already populated.
# Output: bench-results/paid-fb-Nx3-$TS/{MODE-rN.log, SUMMARY.txt}
#
# Usage:
#   ROUNDS=6 ./run-paid-fb-Nx3.sh    # 6 rounds × 3 modes = 18 runs
#   ROUNDS=3 ./run-paid-fb-Nx3.sh    # default
set -uo pipefail
...
OUT_DIR="$PROJECT_ROOT/bench-results/paid-fb-${ROUNDS}x3-$TS"
mkdir -p "$OUT_DIR"
...
run_one() {
    ...
    MODE="$mode" ./run-bench-fb.sh > "$logfile" 2>&1
    ...
    # Thermal cool-down between runs (lets macOS turbo headroom recover)
    echo "  [cool-down ${COOL_DOWN}s]" | tee -a "$SUMMARY"
    sleep "$COOL_DOWN"
}
...
if [ "$ART_COUNT" -ge 3 ]; then
    ...
    AOT_PREWARMUP=1 MODE=aot ./run-bench-fb.sh > "$OUT_DIR/fb-prewarmup.log" 2>&1
    ART_COUNT=$(ls "$AOT_DIR"/*.so 2>/dev/null | wc -l | tr -d ' ')
