...
impl<S> LegacyRpcRouterService<S> {
    ...
    /// Forwards a batch of requests to the legacy endpoint as a single JSON-RPC batch request.
    /// Returns a vector of responses corresponding to each request, maintaining order.
    async fn forward_batch_to_legacy(
        &self,
        requests: Vec<(Request<'_>, usize)>, // Request and its original position in batch
    ) -> Vec<(MethodResponse, usize)> {
        if requests.is_empty() {
            return Vec::new();
        }

        // Build JSON-RPC batch request body
        let batch_body: Vec<serde_json::Value> = requests
            .iter()
            .enumerate()
            .map(|(idx, (req, _))| {
                serde_json::json!({
                    ...
                    "params": req.params().as_str()
                        .and_then(|s| serde_json::from_str::<serde_json::Value>(s).ok())
                        .unwrap_or(serde_json::Value::Null),
                    "id": idx  // Use index as ID to match responses
                })
            })
            .collect();

        match self.client.post(&self.config.legacy_endpoint).json(&batch_body).send().await {
            Ok(response) => match response.json::<Vec<serde_json::Value>>().await {
                Ok(json_responses) => {
                    // Map responses back to their requests
                    requests
                        .into_iter()
                        .enumerate()
                        .map(|(idx, (req, original_pos))| {
                            let request_id = req.id().clone();

                            // Find the corresponding response by id
                            let json_response = json_responses.iter().find(|resp| {
                                resp.get("id")
                                    .and_then(|id| id.as_u64())
                                    .map(|id| id as usize == idx)
                                    .unwrap_or(false)
                            });

                            let method_response = if let Some(json) = json_response {
                                if let Some(result) = json.get("result") {
                                    let payload =
                                ...
                                // Response not found for this request
                                MethodResponse::error(
                                    request_id,
                                    ErrorObject::owned(
                                        INTERNAL_ERROR_CODE,
                                        "No response from legacy endpoint for this request",
                                        None::<()>,
                                    ),
                                )
                            };

                            (method_response, original_pos)
                        })
                        .collect()
                }
                Err(e) => {
                    // Parse error - return errors for all requests
                    requests
                        .into_iter()
                        .map(|(req, original_pos)| {
                            let response = MethodResponse::error(
                                req.id().clone(),
                                ErrorObject::owned(
                                    INTERNAL_ERROR_CODE,
                                    format!("Legacy batch parse error: {e}"),
                                    None::<()>,
                                ),
                            );
                            (response, original_pos)
                        })
                        .collect()
                }
            },
            Err(e) => {
                tracing::error!(target: "rpc::legacy", error = %e, "Legacy RPC batch request failed");
                // Network error - return errors for all requests
                requests
                    .into_iter()
                    .map(|(req, original_pos)| {
                        let response = MethodResponse::error(
                            ...
                            ErrorObject::owned(
                                INTERNAL_ERROR_CODE,
                                format!("Legacy RPC batch error: {e}"),
                                None::<()>,
                            ),
                        );
                        (response, original_pos)
