...
use crate::pre_exec_types::{PreExecError, PreExecInnerTx, PreExecResult};
...
pub trait PreExec: EthCall {
    ...
    fn convert_call_tracer_to_inner_txs(
        ...
        helpers::convert_call_tracer_to_inner_txs(call_frame)
    }
    ...
    where
        ...
        <DB as revm::Database>::Error: std::fmt::Debug,
    {
        helpers::validate_pre_args(db, tx, prev, index)
    }

    /// Classify error message
    fn classify_error(msg: String) -> PreExecError {
        helpers::classify_error(msg)
    }
}
