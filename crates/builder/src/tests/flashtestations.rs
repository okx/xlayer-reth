...
use crate::{
    ...
    tests::{
        ...
        flashtestation_registry::FlashtestationRegistry, BlockTransactionsExt, BundleOpts,
        ChainDriver, ChainDriverExt, LocalInstance, TransactionBuilderExt,
        BLOCK_BUILDER_POLICY_ADDRESS, FLASHBLOCKS_NUMBER_ADDRESS, FLASHTESTATION_REGISTRY_ADDRESS,
        MOCK_DCAP_ADDRESS, TEE_DEBUG_ADDRESS,
    },
};
...
async fn test_flashtestations_with_number_contract(rbuilder: LocalInstance) -> eyre::Result<()> {
    ...
    assert_eq!(count_txs_to(&txs, BLOCK_BUILDER_POLICY_ADDRESS), 1, "Should have 1 block proof tx");
...
async fn test_flashtestations_permit_with_flashblocks_number_permit(
    ...
    assert_eq!(count_txs_to(&txs, Address::ZERO), 2, "Should have 2 regular builder txs");
