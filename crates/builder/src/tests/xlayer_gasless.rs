...
fn gasless_node_config_opt(
    ...
    // Genesis block base fee, parameterized per test. Most gasless tests pass 0 (a fixed point
    // under EIP-1559) so the pool's best-iterator yields the zero-priced tx
    // (`max_fee_per_gas (0) >= base_fee (0)`): gasless execution itself does not need this — the
    // base-fee check is disabled for gasless txs — it only lets the 0-price tx through the pool's
    // fee filter. A non-zero base fee is used by tests that supply the gasless tx via payload
    // attributes (`no_tx_pool=true`, bypassing the pool filter) to prove gasless execution still
    // succeeds over a base fee a non-gasless zero-priced tx could not clear. (A base fee of 1 is
    // itself a fixed point: the EIP-1559 step rounds to 0 at base fee 1, so it cannot decay to 0.)
