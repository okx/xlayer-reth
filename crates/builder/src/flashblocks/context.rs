...
use reth_optimism_evm::{
    GaslessContract, GaslessFeeHook, OpEvmConfig, OpNextBlockEnvAttributes, OpTx,
    XLayerGaslessFeeHook,
};
...
impl FlashblocksBuilderCtx {
    ...
    fn transact_maybe_gasless<DB>(
        ...
        (ResultAndState<<OpEvm<DB, NoOpInspector, PrecompilesMap, OpTx> as Evm>::HaltReason>, bool),
        <OpEvm<DB, NoOpInspector, PrecompilesMap, OpTx> as Evm>::Error,
    >
    where
        DB: Database,
    {
    ...
    fn is_gasless<DB>(
        ...
        evm: &mut OpEvm<DB, NoOpInspector, PrecompilesMap, OpTx>,
        tx: &Recovered<OpTransactionSigned>,
    ) -> Result<bool, <OpEvm<DB, NoOpInspector, PrecompilesMap, OpTx> as Evm>::Error>
    where
        DB: Database,
    {
