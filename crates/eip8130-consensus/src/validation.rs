...
///
/// XLayer uses `from: Option<Address>` — `None` means EOA mode.
...
mod tests {
    ...
    fn structure_validation_too_many_calls() {
        ...
        assert!(matches!(
            ...
            Err(ValidationError::TooManyCalls { count, limit })
    ...
    fn structure_validation_too_many_account_changes() {
        ...
        assert!(matches!(
            validate_structure(&tx),
            Err(ValidationError::TooManyAccountChanges { count, limit })
    ...
    fn structure_validation_rejects_too_many_config_operations() {
        ...
        assert!(matches!(
            validate_structure(&tx),
            Err(ValidationError::TooManyConfigOperations { count, limit })
