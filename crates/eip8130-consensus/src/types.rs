...
impl OwnerScope {
    ...
    /// Can authorize configuration changes.
...
/// Authorize owner operation type.
...
pub enum AccountChangeEntry {
    ...
    /// Set EIP-7702-style code delegation.
    ...
    fn decode(buf: &mut &[u8]) -> alloy_rlp::Result<Self> {
        ...
        match type_byte {
            CHANGE_TYPE_CREATE => {
                ...
                if owners_header.payload_length > buf.len() {
            ...
            CHANGE_TYPE_CONFIG => {
                ...
                if ops_header.payload_length > buf.len() {
                    return Err(alloy_rlp::Error::InputTooShort);
                }
...
mod tests {
    ...
    fn delegation_entry_rlp_round_trip() {
        let entry =
            AccountChangeEntry::Delegation(DelegationEntry { target: Address::repeat_byte(0xCC) });
        let mut buf = Vec::new();
        entry.encode(&mut buf);
        let decoded = AccountChangeEntry::decode(&mut buf.as_slice()).unwrap();
        assert_eq!(entry, decoded);
