//! XLayerAA execution-time transaction parts.
//!
//! These types mirror the subset of an XLayerAA (EIP-8130) wire transaction
//! that the revm handler needs for phased call execution, auto-delegation,
//! and pre-execution storage writes. They use only revm primitives to avoid
//! a circular dependency on any consensus-layer wire crate.
...
/// Trait extending an op-revm transaction with XLayerAA (EIP-8130) execution parts.
///
/// Used by the handler, precompiles, and config helpers to access AA-specific
/// fields without naming the concrete wire type. Extends [`OpTxTr`] so the
/// non-AA flow continues to see the full op-stack transaction surface.
#[auto_impl::auto_impl(&, &mut, Box, Arc)]
pub trait XLayerAATxTr: OpTxTr {
    /// Returns the XLayerAA execution parts for this transaction.
    ///
    /// Non-AA transactions must return a default (empty) instance.
    fn xlayeraa_parts(&self) -> &XLayerAAParts;
}

/// A single call within an XLayerAA transaction phase.
#[derive(Clone, Debug, Default, PartialEq, Eq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct XLayerAACall {
    /// Target address.
    pub to: Address,
    /// Calldata.
    pub data: Bytes,
...
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct XLayerAAStorageWrite {
...
#[derive(Clone, Debug, PartialEq, Eq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct XLayerAACodePlacement {
...
#[derive(Clone, Debug, PartialEq, Eq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct XLayerAAPhaseResult {
...
#[derive(Clone, Debug, PartialEq, Eq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct XLayerAASequenceUpdate {
...
/// Aggregated XLayerAA execution data populated during transaction conversion.
///
/// Built from the wire `TxXLayerAA` in the evm-compat layer and consumed by
/// the handler during execution. Non-AA transactions carry a default (empty)
/// instance.
#[derive(Clone, Debug, Default, PartialEq, Eq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct XLayerAAParts {
    ...
    /// Full XLayerAA intrinsic gas cost (protocol-computed, non-refundable).
    ...
    pub pre_writes: Vec<XLayerAAStorageWrite>,
    ...
    pub config_writes: Vec<XLayerAAStorageWrite>,
    ...
    pub sender_verify_call: Option<XLayerAAVerifyCall>,
    /// Pre-encoded STATICCALL for custom payer verifier. Same semantics.
    pub payer_verify_call: Option<XLayerAAVerifyCall>,
    ...
    pub authorizer_validations: Vec<XLayerAAAuthorizerValidation>,
    ...
    pub account_creation_logs: Vec<XLayerAAConfigLog>,
    ...
    pub config_change_logs: Vec<XLayerAAConfigLog>,
...
#[derive(Clone, Debug, Default, PartialEq, Eq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct XLayerAAAuthorizerValidation {
    ...
    pub verify_call: Option<XLayerAAVerifyCall>,
...
/// Encodes phase results into the output bytes of an XLayerAA transaction.
///
/// Format: one byte per phase, `0x01` = success, `0x00` = failure.
pub fn encode_phase_statuses(results: &[XLayerAAPhaseResult]) -> Bytes {
    Bytes::from(results.iter().map(|r| u8::from(r.success)).collect::<Vec<_>>())
}

/// Decodes phase statuses from XLayerAA transaction output bytes.
...
///
/// `keccak256("XLayerAAPhaseStatuses(bytes)")`
pub fn phase_statuses_log_topic() -> B256 {
    keccak256(b"XLayerAAPhaseStatuses(bytes)")
}
...
/// Converts an [`XLayerAAConfigLog`] into a revm [`Log`] emitted from
/// `emitter` (the AccountConfiguration contract address).
pub fn config_log_to_system_log(emitter: Address, event: &XLayerAAConfigLog) -> Log {
    match event {
        XLayerAAConfigLog::OwnerAuthorized { account, owner_id, verifier, scope } => {
        ...
        XLayerAAConfigLog::OwnerRevoked { account, owner_id } => Log {
            address: emitter,
            data: LogData::new_unchecked(
        ...
        XLayerAAConfigLog::AccountCreated { account, user_salt, code_hash } => {
            let mut data = Vec::with_capacity(64);
...
pub fn phase_statuses_system_log(emitter: Address, results: &[XLayerAAPhaseResult]) -> Log {
...
/// Extracts per-phase statuses from a system log emitted during XLayerAA execution.
///
/// Scans receipt logs for the `XLayerAAPhaseStatuses` topic from the expected emitter address.
...
mod tests {
    ...
    fn encode_all_success() {
        let results = vec![
            XLayerAAPhaseResult { success: true, gas_used: 100 },
            XLayerAAPhaseResult { success: true, gas_used: 200 },
        ];
    ...
    fn encode_all_failure() {
        let results = vec![
            XLayerAAPhaseResult { success: false, gas_used: 50 },
            XLayerAAPhaseResult { success: false, gas_used: 75 },
        ];
        let encoded = encode_phase_statuses(&results);
    ...
    fn encode_mixed() {
        let results = vec![
            XLayerAAPhaseResult { success: true, gas_used: 10 },
            XLayerAAPhaseResult { success: false, gas_used: 20 },
            XLayerAAPhaseResult { success: true, gas_used: 30 },
        ];
        let encoded = encode_phase_statuses(&results);
    ...
    fn decode_roundtrip() {
        let results = vec![
            XLayerAAPhaseResult { success: true, gas_used: 0 },
            XLayerAAPhaseResult { success: false, gas_used: 0 },
            XLayerAAPhaseResult { success: true, gas_used: 0 },
            XLayerAAPhaseResult { success: false, gas_used: 0 },
        ];
        let encoded = encode_phase_statuses(&results);
    ...
    fn parts_default_is_empty() {
        let parts = XLayerAAParts::default();
    ...
    fn owner_authorized_log_encoding() {
        ...
        let event = XLayerAAConfigLog::OwnerAuthorized { account, owner_id, verifier, scope };
    ...
    fn owner_revoked_log_encoding() {
        ...
        let event = XLayerAAConfigLog::OwnerRevoked { account, owner_id };
        let log = config_log_to_system_log(emitter, &event);
    ...
    fn account_created_log_encoding() {
        ...
        let event = XLayerAAConfigLog::AccountCreated { account, user_salt, code_hash };
        let log = config_log_to_system_log(emitter, &event);
    ...
    fn sequence_update_preserves_lock_bits() {
        ...
        let updated =
            XLayerAASequenceUpdate { slot: U256::ZERO, is_multichain: true, new_value: 99 }
                .apply(current);
