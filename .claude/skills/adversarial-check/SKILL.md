...
**Always save the full report to a file, in addition to presenting it in the conversation.** This is mandatory in every run (interactive or CI) — the CI pipeline uploads it as a downloadable GitLab artifact:

- Path: `$ADVERSARIAL_REPORT_PATH` if that environment variable is set (this is how the CI job pins the artifact location); otherwise default to `adversarial-check-report.md` in the repository root.
- The file is Markdown and must contain the complete report (every section below, all findings in full — not a summary of the chat output). Start it with a title line, the Scope line, the invocation date, and the verdict, so the artifact is self-contained when downloaded without the conversation.
- These report files are the only sanctioned working-tree writes (see Hard constraints). They are untracked output — never `git add` or commit them.
- Write the file BEFORE ending the run, even when the verdict is `NO CONSENSUS RISK FOUND` or the run aborts on invalid refs (in the abort case, write the failure reason so the CI artifact explains itself).

**If the report is also rendered as an HTML page / claude.ai Artifact, save the HTML file locally too.** Some readers cannot open claude.ai artifact links due to account restrictions, so the artifact URL alone is not an acceptable delivery channel — the same page must be downloadable from the GitLab pipeline:

- Whenever this skill publishes the report as a claude.ai Artifact (or otherwise renders an HTML version of the report), it MUST also write that HTML to a local file: `$ADVERSARIAL_REPORT_HTML_PATH` if set, otherwise `adversarial-check-report.html` next to the Markdown report.
- The local HTML file must be a **complete standalone document** that opens directly in a browser with no network access: wrap the page content in a full `<!doctype html><html><head><meta charset="utf-8"><title>…</title>…</head><body>…</body></html>` skeleton (the Artifact publisher adds this wrapper server-side, so the source file you pass to the Artifact tool lacks it — add it to the local copy yourself), and keep all CSS/JS inline with no external URLs, fonts, or CDN references. Verify the saved file contains the full findings content, not a stub or a redirect to the artifact URL.
- The Markdown report remains mandatory and authoritative in every run; the HTML copy is required only when an HTML/Artifact rendering was produced. When both exist, mention both paths (and the artifact URL, if any) in the final chat message.
