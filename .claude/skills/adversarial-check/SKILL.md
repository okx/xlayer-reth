...
**Always save the full report to an HTML file, in addition to presenting it in the conversation.** This is mandatory in every run (interactive or CI) — the CI pipeline uploads it as a downloadable GitLab artifact, and some readers cannot open claude.ai artifact links due to account restrictions, so the artifact URL alone is never an acceptable delivery channel. Do NOT also write a Markdown report file — the HTML file is the single report artifact:

- Path: `$ADVERSARIAL_REPORT_PATH` if that environment variable is set (this is how the CI job pins the artifact location); otherwise default to `adversarial-check-report.html` in the repository root.
- The file must contain the complete report (every section below, all findings in full — not a summary of the chat output). Start it with a title, the Scope line, the invocation date, and the verdict, so the artifact is self-contained when downloaded without the conversation.
- The file must be a **complete standalone document** that opens directly in a browser with no network access: a full `<!doctype html><html><head><meta charset="utf-8"><title>…</title>…</head><body>…</body></html>` skeleton with all CSS/JS inline and no external URLs, fonts, or CDN references. When the report is also published as a claude.ai Artifact, this local file carries the same content — but note the Artifact publisher adds the doctype/head/body wrapper server-side, so the source passed to the Artifact tool lacks it; add the wrapper to the local copy yourself. Verify the saved file contains the full findings content, not a stub or a redirect to the artifact URL.
- This file is the single sanctioned working-tree write (see Hard constraints). It is untracked output — never `git add` or commit it.
- Write the file BEFORE ending the run, even when the verdict is `NO CONSENSUS RISK FOUND` or the run aborts on invalid refs (in the abort case, write the failure reason so the CI artifact explains itself). Mention the file path (and the artifact URL, if one was published) in the final chat message.
